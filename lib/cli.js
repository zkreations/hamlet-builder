  ...
  program
    ...
    .action(async (cliOptions) => {
      ...
      if (mergedOptions.info) {
        console.warn(`Starting ${styleText('green', `${name}@${version}`)} for generating project information\n`)
        await infoMode(mergedOptions)
        return
      }

      console.warn(`Starting ${styleText('green', `${name}@${version}`)} in ${styleText('blue', mergedOptions.mode)} mode\n`)

      if (!fs.existsSync(mergedOptions.input)) {
        console.error(styleText('red', `Error: ${mergedOptions.input} does not exist`))
        process.exit(1)
      }
...
export async function runCli(argv = process.argv) {
  ...
  try {
    ...
    console.error(styleText('red', '[Fatal Error] Execution failed:'), error.message || error)
    process.exit(1)
  }
}
