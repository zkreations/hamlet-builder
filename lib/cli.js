...
const defaultPkgJson = path.join(import.meta.dirname, '../package.json')
...
export function createCli({ cwd = process.cwd(), pkg } = {}) {
  ...
  program
    ...
    .option('--inspect', 'Inspect project partials, configurations, and diagnostics')
    .option('-I, --info', 'Alias for --inspect')
    ...
    .option('-s, --sourcemap', 'Enable source map generation')
    .action(async (cliOptions) => {
      ...
      if (mergedOptions.inspect || mergedOptions.info) {
        logger.hamlet(version, 'inspect')
