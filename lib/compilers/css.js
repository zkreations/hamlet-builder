...
function getCSSLocation(file, error) {
  ...
  // LightningCSS errors expose loc: { line, column }
  if (error.loc) {
 ...
 * @param {object} [targets] - LightningCSS browser targets
 */
async function compileFile(file, options, outputFolder, targets) {
  ...
  try {
    ...
    code = compiled.css
    const postCssMap = isDevelopment ? compiled.map : null
...
export async function compileStyle(options) {
  ...
  try {
    ...
    // Continue without targets if browserslist resolution fails
  }
