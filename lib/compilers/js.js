...
async function compileBundleFile(file, options, outputFolder) {
  ...
  try {
    const userPlugins = options.rollup?.plugins ?? []
    const esbuildPlugin = esbuild({
      ...
      ...options.hamlet?.esbuild,
    })
    const plugins = [esbuildPlugin, ...userPlugins]
    ...
    const bundle = await rollup({
      ...
      onwarn(warning, defaultHandler) {
        if (typeof options.rollup?.onwarn === 'function') {
          options.rollup.onwarn(warning, defaultHandler)
          return
        }
        const loc = warning.loc
          ? `${path.relative(process.cwd(), warning.loc.file ?? file)}:${warning.loc.line}:${warning.loc.column ?? 0}`
          : path.relative(process.cwd(), file)
        logger.warn(warning.message, loc)
      },
    })
    bundleCache.set(file, bundle.cache)

    const { output: [{ code, map }] } = await bundle.generate({
      file: path.join(outputFolder, `${rawFileName}.js`),
      format: 'iife',
      name: iifeName,
      sourcemap: isDevelopment,
    })
    ...
    if (isDevelopment && map) {
      await writeOutput({
        output: outputFolder,
        file: `${rawFileName}.js.map`,
...
export async function compileJS(options) {
  const files = globSync(`${options.input}/**/*.bundle.@(js|mjs|cjs|ts|tsx)`, {
    ignore: GLOB_IGNORED,
  })
