...
export const compileStyle = async (config) => {
  // Search for all css, scss and sass files in the input directory
  const files = globSync(`${config.input}/**/!(_)*.{scss,sass,css}`)

  if (files.length === 0) {
    return
  }

  const outputFolder = path.join(config.output, 'css')
  ...
  for (const file of files) {
    ...
    const plugins = config.postcss.plugins ?? []
    ...
    if (!config.minify || !config.minifyCss) {
      return
    }
