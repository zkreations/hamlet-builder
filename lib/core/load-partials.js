...
// @return object
const extractGroupVariables = (files) => {
...
const collectPartials = (input) => {
  ...
  const groupVariables = extractGroupVariables(files)
...
export const loadPartialsInfo = (input) => {
  const { normalPartials, folderPartials, duplicates, folderDuplicates, hamletPartials } = collectPartials(input)
