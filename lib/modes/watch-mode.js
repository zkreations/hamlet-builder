...
export const watchMode = (options) => {
  ...
  const processPendingCompilations = async () => {
    ...
    const hasTailwindCSS = options.postcss?.plugins?.some(
      plugin => plugin.postcssPlugin?.includes('tailwindcss')
    )

    console.log(`[${currentTime()}] Compiling... ${chalk.yellow(`(${fileNamesLog.join(', ')})`)}`)
    ...
    try {
      ...
      if (requiresStyle || hasTailwindCSS) {
