...
export function watchMode(options) {
  ...
  let activeCompilation = Promise.resolve()
  ...
  const processPendingCompilations = async () => {
    ...
    try {
      ...
      if (pendingRerun && !isClosing) {
        pendingRerun = false
        debounceTimeout = setTimeout(() => {
          activeCompilation = processPendingCompilations()
        }, DEBOUNCE_DELAY)
      }
    }
  }
  ...
  watcher.on('error', (error) => {
    if (isClosing)
      return
    logger.warn(`Watcher error: ${error.message || String(error)}`)
  })
  ...
  watcher.close = async () => {
    ...
    closePromise = activeCompilation
      .catch(() => {})
      .then(() => closeWatcher())
