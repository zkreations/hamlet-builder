...
export async function loadPlugins(plugins = [], existing = {}) {
  ...
  for (const entry of plugins) {
    ...
    try {
      ...
      console.warn(`${styleText('yellow', '[Warning]')} Plugin at index ${index} failed during execution: ${error.message}\n`)
      continue
    }

    if (!isValidPluginShape(plugin)) {
      console.warn(`${styleText('yellow', '[Warning]')} Plugin #${index} must resolve to a plain object. Skipped.\n`)
      continue
    }
    ...
    if (!isValidNamespace(namespace)) {
      console.warn(`${styleText('yellow', '[Warning]')} Plugin #${index} must define a valid namespace. Skipped.\n`)
      continue
    }

    if (namespaces.has(namespace)) {
      console.warn(`${styleText('yellow', '[Warning]')} Duplicate plugin namespace "${namespace}". Skipped.\n`)
      continue
    }
    ...
    for (const [key, value] of Object.entries(plugin.partials ?? {})) {
      if (!isValidPartialName(key)) {
        console.warn(`${styleText('yellow', '[Warning]')} Plugin "${namespace}" partial "${key}" has an invalid name. Skipped.\n`)
        continue
      }

      if (!isValidPartial(value)) {
        console.warn(`${styleText('yellow', '[Warning]')} Plugin "${namespace}" partial "${key}" must be a string. Skipped.\n`)
        continue
      }
      ...
      if (reservedPartials.has(fullName)) {
        console.warn(`${styleText('yellow', '[Warning]')} Plugin "${namespace}" tried to register partial "${fullName}", which already exists. Skipped.\n`)
        continue
      }
    ...
    for (const [key, value] of Object.entries(plugin.helpers ?? {})) {
      if (!isValidHelperName(key)) {
        console.warn(`${styleText('yellow', '[Warning]')} Plugin "${namespace}" helper "${key}" has an invalid name. Skipped.\n`)
        continue
      }

      if (!isValidHelper(value)) {
        console.warn(`${styleText('yellow', '[Warning]')} Plugin "${namespace}" helper "${key}" must be a function. Skipped.\n`)
        continue
      }
      ...
      if (reservedHelpers.has(fullName)) {
        console.warn(`${styleText('yellow', '[Warning]')} Plugin "${namespace}" tried to register helper "${fullName}", which already exists. Skipped.\n`)
        continue
      }
    ...
    if ('context' in plugin) {
      if (!isValidContext(plugin.context)) {
        console.warn(`${styleText('yellow', '[Warning]')} Plugin "${namespace}" context must be a plain object. Ignored.\n`)
      }
      else {
