...
export function lintBloggerTemplate(template, _options = {}) {
  ...
  const hasSkin = template.includes('<b:skin') || template.includes('<b:template-skin')
  if (hasSkin) {
  ...
  for (const match of template.matchAll(widgetRegex)) {
    const attrs = match[1]
  ...
  for (const match of template.matchAll(sectionRegex)) {
    const attrs = match[1]
    const id = getAttr(attrs, 'id')
    const index = match.index ?? 0
  ...
  for (const match of template.matchAll(tagTokenRegex)) {
    const tag = match[0]
    const index = match.index ?? 0
  ...
  for (const containerMatch of template.matchAll(containerBlockRegex)) {
