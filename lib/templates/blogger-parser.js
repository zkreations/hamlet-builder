...
import { resolveDocumentDefaultMarkups } from './defaultmarkups-resolver.js'
import { processIncludeElement } from './include-attributes.js'
import {
  normalizeBloggerTag,
  normalizeExprAttributes,
} from './normalizer.js'
import { stripRootDirectives } from './root-directives.js'
import {
...
export {
  formatDiagnostics,
  lintBloggerTemplate,
} from './blogger-linter.js'
export { resolveDocumentDefaultMarkups } from './defaultmarkups-resolver.js'
...
function rootAttributes(element) {
  ...
  const withAdditions = additions ? element.replace('>', `${additions}>`) : element
  return stripRootDirectives(withAdditions)
}
...
export function processTemplate(template, options = {}) {
  const TYPES = {}
  ...
  const markupsResolved = resolveDocumentDefaultMarkups(withExpanded, options)
  const widgetExpanded = expandWidgetIncludables(markupsResolved)
  const skinExpanded = expandSkinShorthands(widgetExpanded)
  const normalized = normalizeExprAttributes(skinExpanded)
