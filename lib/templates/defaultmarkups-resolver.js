 ...
 * preserves custom types like 'Common', and respects the h:resolveMarkups='false' directive.
 */
export function resolveDocumentDefaultMarkups(template) {
  if (typeof template !== 'string' || !template.includes('<b:defaultmarkups'))
    return template

  // Check for root directive h:resolveMarkups='false'
  const htmlMatch = template.match(/<html\b([^>]*)>/i)
  if (htmlMatch) {
    const htmlAttrs = htmlMatch[1]
    const resolveAttr = getAttrValue(htmlAttrs, 'h:resolveMarkups')
    if (resolveAttr === 'false') {
      // Strip h:resolveMarkups attribute and skip normalization
      const cleanedHtmlTag = htmlMatch[0].replace(/\s*h:resolveMarkups=(['"])[^'"]*\1/i, '')
      return template.replace(htmlMatch[0], cleanedHtmlTag)
    }
  }
  ...
  // Find all <b:defaultmarkups> blocks
  ...
  for (const groupKey of NATIVE_GROUPS) {
    ...
    for (const missingId of missing) {
      blockIncludables.push(`    <b:includable id='${missingId}'/>`)
    }

    consolidatedBlocks.push(
      `  <b:defaultmarkup type='${groupKey}'>\n${blockIncludables.join('\n')}\n  </b:defaultmarkup>`,
    )
  }
  ...
  // Strip h:resolveMarkups from <html> if present
  const finalHtml = cleanedResult.replace(/<html\b[^>]*>/i, htmlTag =>
    htmlTag.replace(/\s*h:resolveMarkups=(['"])[^'"]*\1/i, ''))
