...
export function createHelpers({
  ...
  function loadAssetContent(resolvedPath, assetLabel) {
    if (!isPathAllowed(resolvedPath)) {
      console.warn(`${chalk.yellow('[Warning]')} Access denied outside project: "${assetLabel}"\n`)
    ...
    if (!isAllowedExtension(resolvedPath)) {
      ...
      console.warn(`${chalk.yellow('[Warning]')} File type not allowed: "${ext}" in "${assetLabel}"\n`)
      return new Handlebars.SafeString(`/* File type not allowed: ${assetLabel} */`)
    }

    if (activeAssets.has(resolvedPath)) {
      console.warn(`${chalk.yellow('[Warning]')} Circular reference detected: "${assetLabel}"\n`)
    ...
    try {
      ...
      if (content.error) {
        console.warn(`${chalk.yellow('[Warning]')} ${content.error}: "${assetLabel}"\n`)
  ...
  function assetCss(name) {
    if (!resolvedOutput) {
      console.warn(`${chalk.yellow('[Warning]')} Output path not configured for assetCss("${name}")\n`)
  ...
  function assetJs(name) {
    if (!resolvedOutput) {
      console.warn(`${chalk.yellow('[Warning]')} Output path not configured for assetJs("${name}")\n`)
      return new Handlebars.SafeString(`/* Output path not configured: ${name} */`)
    }
