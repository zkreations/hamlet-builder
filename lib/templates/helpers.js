...
import { capitalize, getAsset, logger } from '../utils/index.js'
import { findHelperCallInAst, formatInclusionStack, formatLocation } from './diagnostics.js'
 ...
 * @param {Array<object>} [options.renderStack] - Execution stack for diagnostic tracking
...
export function createHelpers({
  ...
  function resolveHelperLocation(name) {
    if (!renderStack || renderStack.length === 0)
      return null

    const currentFrame = renderStack[renderStack.length - 1]
    const loc = currentFrame.ast ? findHelperCallInAst(currentFrame.ast, name) : null
    const primaryLoc = formatLocation(currentFrame.file, loc)
    const inclusions = formatInclusionStack(renderStack)

    return [primaryLoc, ...inclusions]
  }

  function helperMissing(...args) {
    const options = args.pop()
    const name = options?.name || 'unknown'
    const locationLines = resolveHelperLocation(name)
    logger.warn(`The helper {{${name}}} does not exist`, locationLines)
    return ''
  }
  ...
  function blockHelperMissing(...args) {
    ...
    const name = options?.name || 'unknown'
    const locationLines = resolveHelperLocation(name)
    logger.warn(`The block helper {{#${name}}} does not exist`, locationLines)
    return ''
  }
