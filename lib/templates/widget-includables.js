 ...
 * grouping includable IDs by widget type.
 */
export function collectDocumentDefaultMarkups(template) {
  ...
  for (const match of template.matchAll(defaultMarkupRegex)) {
    ...
    for (const incMatch of content.matchAll(includableRegex)) {
      ...
      if (id) {
        includables.add(id.trim())
      }
    }

    for (const t of types) {
      if (!themeMap.has(t)) {
        themeMap.set(t, new Set())
      }
      const existing = themeMap.get(t)
      for (const incId of includables) {
        existing.add(incId)
      }
    }
  }
...
export function resolveWidgetIncludables(widgetType, themeDefaultMarkups = new Map()) {
  ...
  if (themeDefaultMarkups.has('All')) {
    for (const id of themeDefaultMarkups.get('All')) {
      result.add(id)
    }
  }
  ...
  if (themeDefaultMarkups.has(widgetType)) {
    for (const id of themeDefaultMarkups.get(widgetType)) {
      result.add(id)
    }
  }
...
function parseWidgetContent(content) {
  ...
  return {
    directContent: hasDirectContent ? withoutIncludables.trim() : null,
    explicitIncludables,
    hasDirectContent,
    widgetSettings,
  }
...
export function expandWidgetIncludables(template) {
  ...
  const result = safeTemplate.replace(widgetRegex, (fullMatch, rawAttrs, rawContent) => {
    ...
    if (isSelfClosing) {
      ...
      for (const [incId, targetName] of overrides) {
        generatedIncludables.push(
          `  <b:includable id='${incId}'>\n    <b:include name='${targetName}'/>\n  </b:includable>`,
        )
      }

      // 2. Neutralize remaining resolved includables
      const neutralized = []
      for (const incId of resolved) {
        if (!overrides.has(incId)) {
      ...
      for (const incId of neutralized) {
        generatedIncludables.push(`  <b:includable id='${incId}'/>`)
      }
    ...
    if (widgetSettings) {
      childrenBlocks.push(`  ${widgetSettings.trim()}`)
    }
    ...
    if (overrides.has('main')) {
      ...
      childrenBlocks.push(`  ${explicitIncludables.get('main').trim()}`)
    }
    ...
    for (const [incId, tagText] of explicitIncludables) {
      if (incId === 'main')
        continue
      childrenBlocks.push(`  ${tagText.trim()}`)
    }

    // 5. Neutralize remaining resolved includables
    ...
    for (const incId of neutralized) {
      childrenBlocks.push(`  <b:includable id='${incId}'/>`)
    }
