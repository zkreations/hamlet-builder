...
export const getConfig = async (fileName) => {
  const explorer = cosmiconfig(fileName)
...
// Check if the attribute is valid for the widget
// @param {String} type - The widget type
// @return {Boolean} - True if the attribute is valid
export function checkWidgetType (type) {
  const validTypes = [
    'AdSense', 'Blog', 'BlogArchive', 'BlogList', 'BlogSearch',
    'BloggerButton', 'ContactForm', 'FeaturedPost', 'Feed',
    'Header', 'HTML', 'Image', 'Label', 'LinkList', 'PageList',
    'PopularPosts', 'Profile', 'ReportAbuse', 'Stats', 'Subscribe',
    'Text', 'TextList', 'Translate', 'Wikipedia'
  ]
