...
describe('logger', () => {
  ...
  it('prints reload message with badge format and content', () => {
  ...
  it('prints built and ready messages with expected content', () => {
    logger.built('completed in 10ms')
    const builtOutput = warnSpy.mock.calls[0][0]
    expect(builtOutput).toContain('completed in 10ms')

    logger.ready('in 10ms')
    const readyOutput = warnSpy.mock.calls[1][0]
    expect(readyOutput).toContain('in 10ms')
  })
